...
class ImageGenerationRequest(BaseModel):
    ...
    n: int | None = Field(default=1, ge=1, le=10, description="Number of images to generate")
    size: str | None = Field(
        default=None, description="Image dimensions in WIDTHxHEIGHT format (e.g., '1024x1024', uses model defaults if omitted)"
    )
    ...
    guidance_scale: float | None = Field(
        default=None, ge=0.0, le=20.0, description="Classifier-free guidance scale (uses model defaults if not specified)"
    )
